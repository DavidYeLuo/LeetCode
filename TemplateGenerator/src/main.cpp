...
int main(int argc, char *argv[]) {
  ...
  // Generate files for test cases
  std::ostringstream oss_test;
  std::ifstream fin_test_template;
  ...
  fin_test_template.close();

  std::ifstream fin_test;
  fin_test.open(testFilePath);
  response = "";
  if (fin_test.is_open()) {
    while (response != "y" && response != "n" && response != "Y" &&
           response != "N") {
      response = UI::prompt("Override test file?(y/n): ");
    }
    if (response == "n" || response == "N") {
      printf("Closing.");
      fin_test.close();
  ...
  fin_test.close();
  fout_test.open(testFilePath);
  temp = std::regex_replace(oss_test.str(), std::regex("@LEETCODENAME@"),
                            leetCodeName);
  fout_test << temp;
  fout_test.close();

  printf("Added %s to %s\n", leetCodeName.c_str(), difficulty.c_str());
  printf("TODO: Edit CMakeLists.txt and README.\n");
  return 0;
}
